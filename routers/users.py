...
def list_users(db: Session = Depends(get_db), _admin: Usuario = Depends(require_admin)):
    return db.query(Usuario).all()


@router.put("/{user_id}/rol", response_model=UsuarioOut,
    summary="Cambiar el rol de un usuario",
    description="Solo accesible para Administradores. Cambia el rol entre 'pm' y 'admin'.")
def update_rol(user_id: int, payload: RolUpdateRequest, db: Session = Depends(get_db), _admin: Usuario = Depends(require_admin)):
...
@router.put("/{user_id}/desactivar", response_model=UsuarioOut,
    summary="Desactivar un usuario",
    description="Solo accesible para Administradores.")
def deactivate_user(user_id: int, db: Session = Depends(get_db), _admin: Usuario = Depends(require_admin)):
    user = db.query(Usuario).filter(Usuario.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Usuario no encontrado")
